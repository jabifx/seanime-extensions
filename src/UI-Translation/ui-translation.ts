...
Currently Watching;Viendo actualmente;En cours de visionnage;Attualmente in visione;Aktuell angesehen;Atualmente assistindo;現在視聴中;현재 시청 중;当前观看中;目前觀看中
Layout;Diseño;Disposition;Layout;Layout;Layout;レイアウト;레이아웃;布局;版面
Select layout;Seleccionar diseño;Sélectionner une disposition;Seleziona layout;Layout auswählen;Selecionar layout;レイアウトを選択;레이아웃 선택;选择布局;選擇版面
Customize the settings for this home item.;Personaliza los ajustes de este elemento de inicio.;Personnalisez les paramètres de cet élément d'accueil.;Personalizza le impostazioni per questo elemento della home.;Passen Sie die Einstellungen für dieses Startelement an.;Personalize as configurações deste item inicial.;このホーム項目の設定をカスタマイズ;홈 항목의 설정을 사용자 지정;自定义此主页项目的设置;自訂此主頁項目的設定
Configure Anime library;Configurar biblioteca de anime;Configurer la bibliothèque d'anime;Configura la libreria di anime;Anime-Bibliothek konfigurieren;Configurar biblioteca de animes;アニメライブラリを設定;애니메이션 라이브러리 구성;配置动漫库;設定動畫資料庫
Enter text;Introduce texto;Entrez du texte;Inserisci testo;Text eingeben;Insira o texto;テキストを入力;텍스트 입력;输入文本;輸入文字
Text;Texto;Texte;Testo;Text;Texto;テキスト;텍스트;文本;文字
Grid;Cuadrícula;Grille;Griglia;Raster;Grade;グリッド;그리드;网格;網格
Seanime will use cached data instead of making API requests.;Seanime usará datos en caché en lugar de hacer solicitudes a la API.;Seanime utilisera les données en cache au lieu de faire des requêtes API.;Seanime utilizzerà i dati memorizzati nella cache invece di effettuare richieste API.;Seanime wird zwischengespeicherte Daten verwenden, anstatt API-Anfragen zu stellen.;O Seanime usará dados em cache em vez de fazer solicitações à API.;SeanimeはAPIリクエストを行う代わりにキャッシュデータを使用します。;Seanime은 API 요청 대신 캐시된 데이터를 사용합니다.;Seanime 将使用缓存的数据而不是发出 API 请求。;Seanime 將使用快取的資料而不是發出 API 請求。
By default, all requests made to AniList are cached. This allows Seanime to keep being usable when AniList goes down. The cache directory is modifiable in the config file.;De forma predeterminada, todas las solicitudes realizadas a AniList se almacenan en caché. Esto permite que Seanime siga siendo utilizable cuando AniList no funcione. El directorio de caché se puede modificar en el archivo de configuración.;Par défaut, toutes les requêtes faites à AniList sont mises en cache. Cela permet à Seanime de rester utilisable lorsque AniList est hors ligne. Le répertoire du cache peut être modifié dans le fichier de configuration.;Per impostazione predefinita, tutte le richieste fatte ad AniList vengono memorizzate nella cache. Ciò consente a Seanime di rimanere utilizzabile quando AniList non è disponibile. La directory della cache è modificabile nel file di configurazione.;Standardmäßig werden alle Anfragen an AniList zwischengespeichert. Dadurch bleibt Seanime nutzbar, wenn AniList ausfällt. Das Cache-Verzeichnis kann in der Konfigurationsdatei geändert werden.;Por padrão, todas as solicitações feitas ao AniList são armazenadas em cache. Isso permite que o Seanime continue utilizável quando o AniList estiver fora do ar. O diretório de cache pode ser modificado no arquivo de configuração.;デフォルトでは、AniListへのすべてのリクエストがキャッシュされます。これにより、AniListがダウンしてもSeanimeを使い続けることができます。キャッシュディレクトリは設定ファイルで変更可能です。;기본적으로 AniList에 대한 모든 요청은 캐시에 저장됩니다. 이를 통해 AniList가 다운되어도 Seanime을 계속 사용할 수 있습니다. 캐시 디렉터리는 설정 파일에서 수정할 수 있습니다.;默认情况下，所有对 AniList 的请求都会被缓存。这使得当 AniList 宕机时，Seanime 仍然可以使用。缓存目录可在配置文件中修改。;預設情況下，所有對 AniList 的請求都會被快取。這使得當 AniList 當機時，Seanime 仍可繼續使用。快取目錄可在設定檔中修改。
Use cache-only mode;Usar modo solo caché;Utiliser le mode cache uniquement;Usa la modalità solo cache;Nur-Cache-Modus verwenden;Usar modo somente cache;キャッシュのみモードを使用;캐시 전용 모드 사용;使用仅缓存模式;使用僅快取模式
Disable AniList caching;Desactivar caché de AniList;Désactiver la mise en cache d’AniList;Disattiva la cache di AniList;AniList-Caching deaktivieren;Desativar cache do AniList;AniListのキャッシュを無効化;AniList 캐시 비활성화;禁用 AniList 缓存;停用 AniList 快取
If enabled, Seanime will stop caching AniList requests to disk.;Si está habilitado, Seanime dejará de almacenar en caché las solicitudes de AniList en el disco.;Si activé, Seanime cessera de mettre en cache les requêtes AniList sur le disque.;Se abilitato, Seanime smetterà di memorizzare nella cache le richieste AniList su disco.;Wenn aktiviert, speichert Seanime keine AniList-Anfragen mehr auf der Festplatte im Cache.;Se ativado, o Seanime deixará de armazenar em cache as solicitações do AniList no disco.;有効にすると、SeanimeはAniListのリクエストをディスクにキャッシュしなくなります。;활성화하면 Seanime은 AniList 요청을 디스크에 캐시하지 않습니다.;启用后，Seanime 将停止将 AniList 请求缓存到磁盘。;啟用後，Seanime 將停止將 AniList 請求快取到磁碟。
`;
